...
export const diffAGT = (
    input: ArchitecturalGroundTruth,
    inputClassified: ClassifiedAGT,
    output: ArchitecturalGroundTruth,
): AGTDiff => {
    const violations: AGTViolation[] = [];
    const inconclusiveFields: string[] = [];
    const hard = new Set(inputClassified.hard_fact_fields);

    for (const field of ['window_count', 'door_count'] as const) {
        if (!hard.has(field)) continue;
    ...
    for (const field of ['has_ceiling_fixture', 'has_built_in_niches'] as const) {
        if (!hard.has(field)) continue;
        if (!input[field].value) continue; // absent-in-input never violates: additions are style-legitimate
        const observed = output[field];
        if (observed.confidence === 'low') {
            inconclusiveFields.push(field);
            continue;
...
const FIELD_LABELS: Record<AGTViolation['field'], string> = {
...
export const buildViolationFeedback = (violations: AGTViolation[]): string => {
    if (violations.length === 0) return '';
    const lines = violations.map(v =>
        `- ${FIELD_LABELS[v.field]}: your previous output showed ${v.observed}, but the source room has ${v.expected}. ${v.detail}.`,
    );
    return [
